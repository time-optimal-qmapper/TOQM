...
QASMparser::~QASMparser() {
	...
	delete in;
    delete[] last_layer;
	for( const auto &p : cregs) {
		int * garbage = p.second.second;
		delete [] garbage;
	}
...
void QASMparser::addUgate(int target, double theta, double phi, double lambda) {
    ...
    snprintf ( g.type, 127, "u3(%f, %f, %f)", theta, phi, lambda);
...
void QASMparser::addSWPgate(int target, int control) {
    gate g;
    unsigned int layer;

    g.target = target;
    g.control = control;
    snprintf ( g.type, 127, "swp");
...
void QASMparser::QASMgate(bool execute) {
	if(sym == Token::Kind::ugate) {
	...
	} else if(sym == Token::Kind::swapgate) {
		scan();
		std::pair<int, int> control = QASMargumentQreg();
		check(Token::Kind::comma);
		std::pair<int, int> target = QASMargumentQreg();
		...
		if(execute) {
			if(control.second == target.second) {
				for(int i = 0; i < target.second; i++) {
					addSWPgate(target.first+i, control.first+i);
				}
			} else if(control.second == 1) {
				for(int i = 0; i < target.second; i++) {
                    addSWPgate(target.first+i, control.first);
				}
			} else if(target.second == 1) {
				for(int i = 0; i < target.second; i++) {
                    addSWPgate(target.first, control.first+i);
				}
			} else {
				std::cerr << "Register size does not match for SWP gate!" << std::endl;
			}
#if VERBOSE
		std::cout << "Applied gate: SWP" << std::endl;
#endif
		}
	} else if(sym == Token::Kind::identifier) {
		scan();
		...
		if(gateIt != compoundGates.end()) {
			...
			if(execute) {
				...
				for(auto it = gateIt->second.gates.begin(); it != gateIt->second.gates.end(); it++) {
					...
					} else if(SWPgate* cx = dynamic_cast<SWPgate*>(*it)) {
						if(argsMap[cx->control].second == argsMap[cx->target].second) {
							for(int i = 0; i < argsMap[cx->target].second; i++) {
                                addSWPgate(argsMap[cx->target].first+i, argsMap[cx->control].first+i);
                            }
						} else if(argsMap[cx->control].second == 1) {
							for(int i = 0; i < argsMap[cx->target].second; i++) {
                                addSWPgate(argsMap[cx->target].first+i, argsMap[cx->control].first);
							}
						} else if(argsMap[cx->target].second == 1) {
							for(int i = 0; i < argsMap[cx->target].second; i++) {
                                addSWPgate(argsMap[cx->target].first, argsMap[cx->control].first+i);
							}
						} else {
							std::cerr << "Register size does not match for SWP gate!" << std::endl;
						}
...
void QASMparser::QASMgateDecl() {
	...
	while(sym != Token::Kind::rbrace) {
		if(sym == Token::Kind::ugate) {
			...
			check(Token::Kind::comma);
			check(Token::Kind::identifier);
			gate.gates.push_back(new SWPgate(control, t.str));
			check(Token::Kind::semicolon);
			...
			for(auto it = g.gates.begin(); it != g.gates.end(); it++) {
				if(Ugate* u = dynamic_cast<Ugate*>(*it)) {
					...
					gate.gates.push_back(new SWPgate(argsMap[cx->control], argsMap[cx->target]));
				} else {
	...
	for(auto it = gate.gates.begin(); it != gate.gates.end(); it++) {
		if(Ugate* u = dynamic_cast<Ugate*>(*it)) {
			...
			std::cout << "  SWP " << cx->control << ", " << cx->target << ";" << std::endl;
		} else {
...
void QASMparser::QASMqop(bool execute) {
	if(sym == Token::Kind::ugate || sym == Token::Kind::cxgate || sym == Token::Kind::swapgate || sym == Token::Kind::identifier) {
		QASMgate(execute);
...
void QASMparser::Parse() {
	...
	do {
		...
		} else if(sym == Token::Kind::ugate || sym == Token::Kind::cxgate || sym == Token::Kind::swapgate || sym == Token::Kind::identifier || sym == Token::Kind::measure || sym == Token::Kind::reset) {
			QASMqop();
